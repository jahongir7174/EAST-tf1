...
from src.nets.base_nets import mobilenet_v3 as mobile_net
from src.utils import config
from src.utils import lanms
from src.utils import restore_rectangle


def get_images():
    files = []
    extensions = ['jpg', 'png', 'jpeg', 'JPG']
    for parent, dir_names, filenames in os.walk(config.test_data_path):
...
def main(argv=None):
    ...
    with tf.get_default_graph().as_default():
        ...
        arg_scope = mobile_net.arg_scope(is_training=False)
        fe_model = mobile_net.mobile_net(input_images)
        ...
        with tf.Session(config=tf.ConfigProto(allow_soft_placement=True)) as sess:
            ...
            for im_fn in im_fn_list:
                ...
                if not config.no_write_images:
                    img_path = os.path.join(config.output_dir, os.path.basename(im_fn))
