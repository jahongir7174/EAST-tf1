...
from src.nets.base_nets import condconv_layers
...
class MBConvBlock(tf.keras.layers.Layer):
    ...
    def __init__(self, block_args, global_params):
        ...
        self.depthwise_conv_cls = src.nets.base_nets.condconv_layers.DepthwiseConv2D
        if self._block_args.condconv:
            ...
            self.depthwise_conv_cls = functools.partial(condconv_layers.DepthwiseCondConv2D,
        ...
        if self._block_args.id_skip:
            if all(
                ...
                if drop_connect_rate:
                    x = src.nets.base_nets.condconv_layers.drop_connect(x, training, drop_connect_rate)
                x = tf.add(x, inputs)
        return x
